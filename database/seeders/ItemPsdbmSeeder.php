...
class ItemPsdbmSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        foreach ($json as $key => $res) {
            ...
            $item['item_subclassification_id'] = isset($res[7]) ? $res[7] : "";
            $item['item_type_id'] = $cse->id;
            
            $parent_lib = Library::where('library_type','user_division')->where('name', $item['item_subclassification_id'])->first();
            $parent_id = null;
            if($parent_lib){
            ...
            $item_subclassification_library = Library::where('name',$item['item_subclassification_id'])->where('library_type', 'item_subclassification')->first();
            ...
            $item['item_subclassification_id'] = $item_subclassification_library ? $item_subclassification_library->id : null;
