...
class OrdFormProcessApproveTest extends TestCase
{
    /**
    ...
    public function test_create()
    {
        ...
        $user_office = Library::with('children')->where('library_type', 'user_division')->where('title','ORD')->first();
        $user_office_id = $this->faker->randomElement($user_office->children()->where('library_type','user_section')->pluck('id'));
        ...
        OrdFormProcessApproveTest::$purchase_request_id = $purchase_request['id'];
    ...
    public function test_approve_user()
    {
        $user = User::with('user_offices.office')->where('username','pas')->first();
        Passport::actingAs($user);
        $form_route = FormRoute::where('status','pending')->where('form_routable_id',OrdFormProcessApproveTest::$purchase_request_id)->first();
    ...
    public function test_update_procurement()
    {
        $user = User::with('user_offices.office')->where('username','procurement')->first();
        Passport::actingAs($user);
        $response = $this->put('/api/purchase-requests/'.OrdFormProcessApproveTest::$purchase_request_id,[
