...
class PswdoFormProcessApproveTest extends TestCase
{
    /**
    ...
    {
        $user = User::with('user_offices.office')->where('username', "like",'pswdo - %')->get();
        $user = $user[$this->faker->numberBetween(0, count($user) - 1)];
        ...
        PswdoFormProcessApproveTest::$purchase_request_id = $purchase_request['id'];
    ...
    public function test_approve_user()
    {
        ...
        Passport::actingAs($user);
        $form_route = FormRoute::where('status','pending')->where('form_routable_id',PswdoFormProcessApproveTest::$purchase_request_id)->first();
    ...
    public function test_update_procurement()
    {
        $user = User::with('user_offices.office')->where('username','procurement')->first();
        Passport::actingAs($user);
        $response = $this->put('/api/purchase-requests/'.PswdoFormProcessApproveTest::$purchase_request_id,[
