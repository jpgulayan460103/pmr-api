...
class DrmdFormProcessApproveTest extends TestCase
{
    /**
    ...
    {
        $user = User::with('user_offices.office')->where('username','drims')->first();
        Passport::actingAs($user);
        $office = $user->user_offices;
        $user_office = Library::with('children')->where('library_type', 'user_division')->where('title','drmd')->first();
        ...
        DrmdFormProcessApproveTest::$purchase_request_id = $purchase_request['id'];
    ...
    public function test_update_procurement()
    {
        $user = User::with('user_offices.office')->where('username','procurement')->first();
        Passport::actingAs($user);
        $response = $this->put('/api/purchase-requests/'.DrmdFormProcessApproveTest::$purchase_request_id,[
    ...
    public function test_approve_drmd()
    {
        $user = User::with('user_offices.office')->where('username','drmd')->first();
        Passport::actingAs($user);
        $form_route = FormRoute::where('status','pending')->where('form_routable_id',DrmdFormProcessApproveTest::$purchase_request_id)->first();
        $response = $this->post('api/forms/routes/requests/pending/'.$form_route->id.'/approve');
        $response->assertStatus(200);
