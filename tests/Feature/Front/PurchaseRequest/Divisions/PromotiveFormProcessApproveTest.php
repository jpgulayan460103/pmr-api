...
class PromotiveFormProcessApproveTest extends TestCase
{
    /**
    ...
    public function test_create()
    {
        ...
        PromotiveFormProcessApproveTest::$purchase_request_id = $purchase_request['id'];
    ...
    public function test_approve_user()
    {
        $user = User::with('user_offices.office')->where('username','slpmo')->first();
        Passport::actingAs($user);
        $form_route = FormRoute::where('status','pending')->where('form_routable_id',PromotiveFormProcessApproveTest::$purchase_request_id)->first();
    ...
    public function test_update_procurement()
    {
        $user = User::with('user_offices.office')->where('username','procurement')->first();
        Passport::actingAs($user);
        $response = $this->put('/api/purchase-requests/'.PromotiveFormProcessApproveTest::$purchase_request_id,[
    ...
    public function test_approve_promotive()
    {
        $user = User::with('user_offices.office')->where('username','promotive')->first();
        Passport::actingAs($user);
        $form_route = FormRoute::where('status','pending')->where('form_routable_id',PromotiveFormProcessApproveTest::$purchase_request_id)->first();
        $response = $this->post('api/forms/routes/requests/pending/'.$form_route->id.'/approve');
        $response->assertStatus(200);
